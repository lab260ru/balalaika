...
### 4.15 Accents / ruAccent (stage 9) — lazy load, thread cap, batching, memo
...
6. ~~**Accents stage (9)** is the slowest text stage per the audit: ruAccent runs
   ...
   see §4.15 (lazy rule-engine skip 1.78× load; thread cap 1.60× at 4 workers;
...
python -m benchmarking.micro.bench_accents --make-fixtures         # §4.15 (once)
python -m benchmarking.micro.bench_accents --impl both --label check          # §4.15 single-worker CPU + 0/250 diffs
python -m benchmarking.micro.bench_accents --impl both --workers 4 --label cap # §4.15 thread-cap win (stock uncapped vs fast capped)
