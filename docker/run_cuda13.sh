...
gpu_devices=${BALALAIKA_GPU_DEVICES:-0}
...
if [[ ! "$gpu_devices" =~ ^[0-9]+(,[0-9]+)*$ ]]; then
    echo "BALALAIKA_GPU_DEVICES must be a comma-separated list of GPU indices" >&2
    exit 2
fi
...
logical_devices=""
for ((index = 0; index < ${#selected_gpus[@]}; index++)); do
...
docker_args=(
    ...
    --env "CUDA_VISIBLE_DEVICES=$logical_devices"
