"""Filter audio using raw Spectra-0 class scores from balalaika.csv.

The filter computes ``spoof_margin = score_spoof - score_bonafide`` and deletes
files whose margin is greater than the selected threshold. A threshold of zero
implements the model's raw argmax decision without sigmoid or softmax.
"""
...
def scored_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out[MARGIN_COLUMN] = out[SCORE_SPOOF_COLUMN] - out[SCORE_BONAFIDE_COLUMN]
    return out
...
def print_distribution(stats: dict) -> None:
    if stats["count"] == 0:
        logger.warning("No complete Spectra-0 score pairs found.")
        return
    print("\n" + "=" * 60)
    print("  Spectra-0 Spoof Margin Distribution")
    print("  margin = score_spoof - score_bonafide")
    print("=" * 60)
...
def determine_threshold(config: dict, args: argparse.Namespace) -> Optional[float]:
    if args.threshold is not None:
        ...
        logger.info(f"Auto mode: using CLI spoof-margin threshold={threshold}")
        return threshold

    cfg = config.get("antispoofing_filter", {})
    configured = cfg.get("threshold")
    if configured is not None and not args.manual:
        threshold = float(configured)
        logger.info(f"Auto mode: using config spoof-margin threshold={threshold}")
        return threshold

    logger.info("Manual mode: interactive spoof-margin threshold selection")
    while True:
        try:
            raw = input(
                "\nEnter spoof-margin threshold "
...
def print_preview(
    ...
    print("\n" + "-" * 72)
    print(f"  Spoof-margin threshold: {threshold:.6f}")
    print(
...
def run_worker(
    ...
    try:
        ...
        logger.exception(f"Anti-spoofing filter worker {rank} failed: {exc}")
        errors_counter.value += 1
...
def run_deletion_workers(
    ...
    try:
        ...
        logger.warning("Anti-spoofing filter interrupted; terminating workers.")
        for process in processes:
            if process.is_alive():
                process.terminate()
        for process in processes:
...
def write_status(
    ...
    write_stage_status(
        stage=6.5,
        stage_name="antispoofing_filter",
...
def main(args):
    setup_logging("antispoofing_filter", log_dir=args.log_dir)
    ...
    if baseline.empty:
        logger.error(
            "No complete score_bonafide/score_spoof pairs in balalaika.csv. "
            "Run stage 6 first."
        )
        write_status(args, processed=0, skipped=0, errors=1)
        return
    ...
    if absorbed:
        logger.info(f"Absorbed {absorbed} leftover anti-spoof filter rows.")
    ...
    if candidates.empty:
        logger.info("No files exceed the spoof-margin threshold.")
    ...
    record_stage_summary(
        ...
        params={
            "score": "score_spoof-score_bonafide",
            "threshold": threshold,
    ...
    logger.success(
        f"Anti-spoofing filter complete. Processed: {processed}, "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Filter audio using raw Spectra-0 spoof/bonafide scores"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override spoof-margin threshold; delete when margin is greater",
    )
