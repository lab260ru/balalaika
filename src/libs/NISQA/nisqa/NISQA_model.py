...
class nisqaModel(object):
    ...
    def _loadDatasetsFolder(self):
        files = list(Path(self.args['data_dir']).rglob('*.wav')) + \
                list(Path(self.args['data_dir']).rglob('*.mp3')) + \
                list(Path(self.args['data_dir']).rglob('*.ogg')) + \
                list(Path(self.args['data_dir']).rglob('*.opus')) + \
                list(Path(self.args['data_dir']).rglob('*.fluc'))
